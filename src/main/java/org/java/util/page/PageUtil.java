...
public class PageUtil {
	...
	public static Map<String, Object> isEmptySetValue(
		...
		if (UtilFuns.isNotEmpty((String) queryMap.get("currentPage"))) {
			...
			//都为空设置默认值
			queryMap.put("currentPage", Integer.parseInt(SystemConfig.getString("currentPage")));
		}
		if (UtilFuns.isEmpty((String) queryMap.get("rows"))) {
			//如果为空设置为默认显示条数
			queryMap.put("rows", Integer.parseInt(SystemConfig.getString("rows")));
		} else {
